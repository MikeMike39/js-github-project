...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document
    ...
    .addEventListener("submit", (event) => search_username(event));
  document
    .querySelector("#repos-form")
    .addEventListener("submit", (event) => search_repos(event));
});
...
const hide_seek = () => {
  ...
  button.addEventListener("click", () => {
    ...
    if (daDom) {
      ...
      button.innerText = "Search by username";
    } else {
      repoForm.style.display = "none";
      gitForm.style.display = "block";
      button.innerText = "Search by repository name";
    }
  });
};
